...
5. 错误不丢数据：错误行产生诊断码（`invalid-escape:行号`、`unclosed-card`、`nested-card:行号`、`invalid-n:行号` 等），同一文件中合法的名片照常返回；
6. `format(Contact) -> String?`：把支持的字段写成 CRLF 结尾的 vCard；版本或姓名无效时返回 `None`，不产出半成品文本；
7. 工程化交付：单元测试覆盖解析、折行、转义、参数保留与错误路径，`moon run` 示例、README 可复现命令、GitHub Actions 检查（`check` / `build` / `test` 与 `fmt`、`info` 工作区一致性），并已发布到 mooncakes.io。
...
**能力边界（明确不做的部分）。** 不支持 quoted-printable、base64 照片、字符集参数、属性分组（如 `item1.TEL`）；`FN` 与 `N` 行上的参数不保留，任意 `VALUE` 类型不解释；未知属性直接忽略，序列化不保留不支持的属性。这是一个"联系人常用文本字段内核"，不是完整 RFC 6350 实现，也不宣称能校验第三方名片的合法性，更不涉及条码、二维码等码字层能力。

**下一阶段可扩展方向。** quoted-printable 解码、按 `PREF` 对多值字段稳定排序。
