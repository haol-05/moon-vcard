...
8. 工程化交付：单元测试覆盖解析、折行、转义、参数保留、quoted-printable 与错误路径，`moon run` 示例、README 可复现命令、GitHub Actions 检查（`check` / `build` / `test` 与 `fmt`、`info` 工作区一致性），并已发布到 mooncakes.io。
